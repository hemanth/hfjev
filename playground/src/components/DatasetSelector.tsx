...
import { Search, Database, ChevronDown, Check, Sparkles, FileText, Upload, AlertCircle, Loader2, Heart, Download } from 'lucide-react';
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        const candidate = parsedRows[0]?.row ? Object.keys(parsedRows[0].row)[0] : 'text';
        onLoadCustomRows(parsedRows, file.name, candidate);
      } catch (err: any) {
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-ink-100 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-ink-900">Dataset Source</h2>
          </div>
            ...
            {PRESET_DATASETS.map((preset) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-[11px] text-ink-600 line-clamp-2 mb-2 leading-relaxed">
                    {preset.description}
                  </p>
                  <div className="mt-auto flex items-center gap-1.5 text-[10px] text-ink-500 font-mono">
                    <code className="truncate max-w-[190px]">{preset.id}</code>
                  </div>
                </button>
              );
            })}
          ...
          <div className="space-y-3">
            <div ref={searchContainerRef} className="relative">
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={() => searchQuery && onSelectDataset(searchQuery.trim())}
            ...
            <form onSubmit={handleCustomDatasetSubmit} className="flex items-center gap-2 pt-1">
              <span className="text-xs text-ink-500 whitespace-nowrap">Or enter exact repository ID:</span>
              <input
                type="text"
                value={customInput}
        ...
        {/* Controls bar: Current active dataset, Column selector, Split, Sample Size */}
        <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-ink-100">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-xs font-medium text-ink-700">Current Dataset:</span>
            <PastelBadge color="lavender" size="md">
              {selectedDataset}
            </PastelBadge>
            {isLoading && (
              <span className="inline-flex items-center gap-1 text-xs text-ink-500">
                <Loader2 size={13} className="animate-spin text-[#7C66DC]" />
          ...
          <div className="flex flex-wrap items-center gap-3">
            {/* Column to classify selector */}
            <div className="flex items-center gap-1.5">
              <label className="text-xs text-ink-600 font-medium">Text Field:</label>
              <select
                value={selectedColumn}
                onChange={(e) => onColumnChange(e.target.value)}
                className="rounded-lg border border-ink-200 bg-white px-2.5 py-1 text-xs font-mono font-medium text-ink-800 focus:border-[#7C66DC] focus:outline-none"
              >
                {features.map((f) => (
                ...
                {features.length === 0 && <option value="text">text</option>}
              </select>
            </div>

            {/* Split Selector */}
            <div className="flex items-center gap-1.5">
              <label className="text-xs text-ink-600 font-medium">Split:</label>
              <select
                value={split}
                onChange={(e) => onSplitChange(e.target.value)}
                className="rounded-lg border border-ink-200 bg-white px-2.5 py-1 text-xs font-medium text-ink-800 focus:border-[#7C66DC] focus:outline-none"
              >
            ...
            <div className="flex items-center gap-1.5">
              <label className="text-xs text-ink-600 font-medium">Rows:</label>
              <select
                value={sampleSize}
